    ...
    private func loadAndMapJSON() {
        ...
        do {
            ...
            self.corridors = rawData.corridors.map {
                CorridorCard(title: $0.title, description: $0.flavor, imageName: $0.title)
            }
            
            // Convert rooms to final "RoomCard"
            self.rooms = rawData.rooms.map {
                RoomCard(title: $0.title, description: $0.flavor, imageName: $0.title)
            }
            ...
            self.puzzles = rawData.puzzles.map { rawPuzzle in
                ...
                """
                return PuzzleCard(title: rawPuzzle.title, description: combined, imageName: "Puzzle")
            }
